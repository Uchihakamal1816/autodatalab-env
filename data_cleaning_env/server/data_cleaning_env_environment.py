...
def _compute_data_quality_score(df: pd.DataFrame) -> float:
    """Fraction of rows passing all business-rule checks.

    Rules:
      - Price column exists and value > 0
      - Quantity column exists and value >= 1
      - OrderDate column exists and is parseable

    Returns a float in [0, 1] giving the agent a continuous quality signal.
    """
    if df.empty:
        return 0.0
    n = len(df)
    passing = pd.Series([True] * n, index=df.index)

    if "Price" in df.columns:
        price_num = pd.to_numeric(df["Price"], errors="coerce")
        passing &= price_num.notna() & (price_num > 0)

    if "Quantity" in df.columns:
        qty_num = pd.to_numeric(df["Quantity"], errors="coerce")
        passing &= qty_num.notna() & (qty_num >= 1)

    if "OrderDate" in df.columns:
        try:
            parsed = pd.to_datetime(df["OrderDate"], errors="coerce")
            passing &= parsed.notna()
        except Exception:
            pass

    return float(passing.sum()) / n
...
TASK_SPECS: Dict[str, Dict[str, Any]] = {
    ...
    "hard": {
        ...
        "max_steps": 30,
        "instruction": (
            "E-commerce Task 3 (Revenue share analysis): clean orders — dedupe, impute missing Price (mean). "
            "Derive Revenue = Price × Quantity. Then run compute_revenue_share to produce a "
            "Category-level table with columns: Category, Revenue, RevenueShare (each category's % of total). "
            "Then submit. Do NOT impute OrderID or CustomerID."
        ),
    },
    "medium_plus": {
        "difficulty": "medium",
    ...
    "expert": {
        ...
        "max_steps": 60,
        "instruction": (
            "E-commerce Task 4 (Expert full pipeline): (1) remove_duplicates, "
            "(2) fill_missing Price with mean, "
            "(3) remove_outliers on Price (MAD z ≤ 2.5), "
            "(4) derive_revenue (Revenue = Price × Quantity), "
            "(5) validate_schema — enforce Price > 0, Quantity ≥ 1, OrderDate parseable, "
            "(6) compute_kpis — produce TotalRevenue and AvgOrderValue summary, "
            "(7) plot scatter OrderDate vs Revenue, "
            "(8) plot bar Category vs Revenue, "
            "(9) plot bar Product vs Revenue, "
            "(10) submit. "
            "Do NOT impute or drop OrderID / CustomerID."
        ),
    },
}
...
class DataCleaningEnvironment(Environment[DataCleaningAction, DataCleaningObservation, State]):
    ...
    def __init__(self) -> None:
        ...
        self._kpi_summary: Optional[pd.DataFrame] = None
    ...
    def _observe(self, initial: bool = False) -> DataCleaningObservation:
        ...
        return DataCleaningObservation(
            ...
            data_quality_score=_compute_data_quality_score(self._df),
        )
        ...
        **kwargs: Any,
        ...
        try:
            if action.action_type == "remove_duplicates":
                ...
                df = self._df
                # In medium_plus, compute_kpis is the *goal* — replace df with KPI table.
                # In expert, compute_kpis is a *side step* — store results without clobbering
                # the cleaned order table that the grader scores against GT.
                if not all(c in df.columns for c in ("Price", "Quantity")):
                    ...
                    if self._task_name == "expert":
                        # Store KPIs as a side-table; keep main df intact for grading.
                        self._kpi_summary = kpi_df
                    else:
                        self._df = kpi_df
                    immediate += 0.18
                    step_summary = (
            ...
            elif action.action_type == "compute_revenue_share":
                df = self._df
                required = ("Price", "Quantity", "Category")
                if not all(c in df.columns for c in required):
                    immediate -= 0.08
                    step_summary = "compute_revenue_share: need Price, Quantity, and Category columns."
                else:
                    t = df.copy()
                    t["Price"] = pd.to_numeric(t["Price"], errors="coerce")
                    t["Quantity"] = pd.to_numeric(t["Quantity"], errors="coerce")
                    rev_col = "Revenue" if "Revenue" in t.columns else None
                    if rev_col:
                        t["_rev"] = pd.to_numeric(t[rev_col], errors="coerce")
                    else:
                        t["_rev"] = t["Price"] * t["Quantity"]
                    agg = (
                        t.groupby("Category", as_index=False)["_rev"]
                        .sum()
                        .rename(columns={"_rev": "Revenue"})
                    )
                    total = agg["Revenue"].sum()
                    if total > 0:
                        agg["RevenueShare"] = (agg["Revenue"] / total * 100).round(2)
                    else:
                        agg["RevenueShare"] = 0.0
                    agg["Revenue"] = agg["Revenue"].round(2)
                    self._df = agg.sort_values("Category").reset_index(drop=True)
                    immediate += 0.20
                    top_cat = agg.loc[agg["Revenue"].idxmax(), "Category"] if len(agg) else "N/A"
                    step_summary = (
                        f"compute_revenue_share: {len(agg)} categories, total revenue={total:,.2f}. "
                        f"Top category: {top_cat}. "
                        "Table now has columns: Category, Revenue, RevenueShare(%)."
                    )
                ...
                if "Price" in df.columns:
                    price_num = pd.to_numeric(df["Price"], errors="coerce")
                    bad_price = int((price_num.notna() & (price_num <= 0)).sum())
                    null_price = int(price_num.isna().sum())
                    if bad_price:
                        violations.append(f"{bad_price} row(s) with Price ≤ 0")
                    if null_price:
                        violations.append(f"{null_price} row(s) with null Price")
                else:
                    violations.append("column 'Price' not found")
                # Rule 2: Quantity must be >= 1
                if "Quantity" in df.columns:
                    qty_num = pd.to_numeric(df["Quantity"], errors="coerce")
                    bad_qty = int((qty_num.notna() & (qty_num < 1)).sum())
                    if bad_qty:
                        violations.append(f"{bad_qty} row(s) with Quantity < 1")
                else:
                    violations.append("column 'Quantity' not found")
                # Rule 3: OrderDate must be parseable
                if "OrderDate" in df.columns:
                    parsed_dates = pd.to_datetime(df["OrderDate"], errors="coerce")
                    bad_dates = int(parsed_dates.isna().sum())
                    if bad_dates:
                        violations.append(f"{bad_dates} row(s) with unparseable OrderDate")
                else:
                    violations.append("column 'OrderDate' not found")

                dq = _compute_data_quality_score(df)
                if not violations:
                    immediate += 0.15
                    step_summary = (
                        f"validate_schema: all rows pass business rules "
                        f"(Price>0, Quantity≥1, OrderDate valid). "
                        f"Data quality score: {dq:.1%}"
                    )
                else:
                    immediate += 0.08
                    step_summary = (
                        f"validate_schema: {len(violations)} rule violation(s) found — "
                        + "; ".join(violations)
                        + f". Data quality score: {dq:.1%}. "
                        "Fix issues before submit for best score."
                    )
