...
def _hard_pipeline_next_action(obs: DataCleaningObservation) -> Optional[DataCleaningAction]:
    """Next step in the hard-task ecommerce pipeline (for stuck recovery)."""
    ...
    if pc == 1:
        return DataCleaningAction(
            ...
            x="Category",
            y="Revenue",
            description="Auto: category sales bar chart.",
        )
    return DataCleaningAction(
        action_type="submit",
...
def _stuck_advance(
    ...
    if at == "fill_missing" and task in ("hard", "expert"):
        return _hard_pipeline_next_action(obs)
...
def _hard_alternate_loop_normalize(
    ...
    """
    Hard task: after cleaning + derive_revenue, skip redundant dedupe/outlier loops and
    advance to the next plot or submit.
    """
    if task != "hard":
        return None
    at = action.action_type
    if at == "submit":
        return None
    if _plot_count(obs) >= 2 and _history_has_action_type(obs, "derive_revenue"):
        return DataCleaningAction(
            action_type="submit",
            description="Auto: both plots declared; submitting hard task.",
        )
    if _fill_missing_count(obs) < 1:
        return None
    ...
    if at in ("noop",):
        return None
    if at in ("remove_duplicates", "remove_outliers", "fill_missing"):
        return _hard_pipeline_next_action(obs)
    return None
