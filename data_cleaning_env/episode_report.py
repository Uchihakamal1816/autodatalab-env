...
def _brief_action_dict(d: Dict[str, Any]) -> str:
    ...
    if at == "compute_revenue_share":
        return "Compute revenue share % per Category (Category, Revenue, RevenueShare)"
    if at == "validate_schema":
        return "Validate schema: enforce Price > 0, Quantity >= 1, OrderDate parseable"
    if at == "export_csv":
...
def _add_preview_table(
    doc: Any,
    records: List[Dict[str, Any]],
    max_rows: int = 15,
) -> None:
    """Add a table. When max_rows is None all rows are included (full CSV export)."""
    ...
    for i, col_name in enumerate(cols):
        ...
        for para in hdr[i].paragraphs:
            for r in para.runs:
                r.font.bold = True
                r.font.size = Pt(9)
    for ri, row in enumerate(rows_to_show, start=1):
...
def _build_single_task_document(trace: EpisodeTrace) -> Any:
    ...
    total_rows = trace.final_row_count or len(trace.final_preview)
    if total_rows <= 500:
        doc.add_heading(f"Full result dataset ({total_rows} rows)", level=1)
        _add_preview_table(doc, trace.final_preview, max_rows=None)
    else:
        doc.add_heading(f"Result dataset preview (first 50 of {total_rows} rows)", level=1)
        _add_preview_table(doc, trace.final_preview, max_rows=50)
...
def write_session_docx(traces: List[EpisodeTrace], path: Path | str) -> Path:
    ...
    for ti, trace in enumerate(traces):
        ...
        total_rows = trace.final_row_count or len(trace.final_preview)
        if total_rows <= 500:
            doc.add_heading(f"Full result dataset ({total_rows} rows)", level=2)
            _add_preview_table(doc, trace.final_preview, max_rows=None)
        else:
            doc.add_heading(f"Result dataset preview (first 50 of {total_rows} rows)", level=2)
            _add_preview_table(doc, trace.final_preview, max_rows=50)
